...
def get_random_message(df, user_column):
    ...
    filtered_df = df[
        df[user_column] == 0
    ]  # Filter to use only rows with user flag == 0
...
def get_user_response(offset=None):
    ...
    response = requests.get(
        f"{URL}/getUpdates", params={"offset": offset, "timeout": 10}
    )
    if response.status_code == 200:
...
def validate_translation(df, row_index, user_id, user_response):
    ...
    if user_response.strip().lower() == correct_translation.strip().lower():
        ...
        df.to_excel(
            DICTIONARY_NAME, index=False
        )  # Save changes to Excel after the correct answer, use False to avoid adding index column
    ...
    for i, other_user_id in enumerate(USER_IDS):
        if other_user_id != user_id:
            ...
            opponent_stats.append(
                f"Your opponent user {i + 1} answered {opponent_correct} questions from {total_words}"
            )
...
def main():
    print(
        f"The program is running. Messages will be sent every {INTERVAL_SECONDS} seconds."
    )

    df = create_dictionary(DICTIONARY_NAME)
    print(
        df.head()
    )  # Verify table structure, print the first 5 rows of dictionary table
        ...
        while True:
            ...
            for i, user_id in enumerate(USER_IDS):
                ...
                if df[user_column].sum() == len(df):
                    send_message(
                        user_id,
                        "Congratulations. You already learn everything.You should add new words.",
                    )
                    print(f"User {i+1} has learned all words.")
                    log_message("Achievement", user_id, "Learned all words.")
                    print(
                        f"We found the Hero! {user_id} is the WINNER! The program will terminate."
                    )
                    exit()
            ...
            for user_id in USER_IDS:
                if (
                    user_current_tasks[user_id] is None
                    and current_time - last_task_time[user_id] >= INTERVAL_SECONDS
                ):
                    random_row = get_random_message(
                        df, f"User {USER_IDS.index(user_id) + 1}"
                    )
                    if random_row is not None:
                        user_current_tasks[
                            user_id
                        ] = random_row.name  # Store the index of the random row
                        send_message(user_id, random_row["Ukrainian"])
                        last_task_time[
                            user_id
                        ] = current_time  # Timestamp, last task time update

                # Check if user has not responded and we want to motive user by statistick sharing
                if (
                    user_current_tasks[user_id] is not None
                    and current_time - last_response_time[user_id] >= TIME_TO_MOTIVATE
                    and MOTIVATION
                ):
                    motivation(user_id)
