    ...
    while True:
        ...
        for user_id in USER_IDS:
            if all_words_learned(df, user_id, USER_IDS):
                ...
                print(
                    f"{user_id} learned all words. Let's congratulate the winner!!! Exiting..."
                )
                exit()

            if (
                user_tasks[user_id] is None
                and now - last_task_time[user_id] >= INTERVAL_SECONDS
            ):
            ...
            if (
                user_tasks[user_id] is not None
                and now - last_response_time[user_id] > TIME_TO_MOTIVATE
                and MOTIVATION
            ):
        ...
        for update in updates:
            ...
            if user_id in USER_IDS and user_tasks[user_id] is not None:
                validate_response(
                    df, user_tasks[user_id], user_id, USER_IDS, text, config, secrets
                )
